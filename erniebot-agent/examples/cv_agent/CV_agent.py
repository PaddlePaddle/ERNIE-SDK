...
async def run_agent():
    await GlobalFileManager().configure(access_token="<your-access-token>")
    ...
    agent_resp = await agent.async_run("这张图片中包含什么中文文字?", files=[ocr_file])  # 单张输入
    print("=" * 10 + "OCR返回结果" + "=" * 10 + "\n")
    print(agent_resp.text)
    print(agent_resp.annotations)
    print("\n" + "=" * 20 + "\n")
    agent_resp = await agent.async_run(
        "请对第一张图片中的物体进行分类然后对第二张图片中的物体进行分割？", files=[clas_file, seg_file]
    )  # 多张输入
    ...
    print(agent_resp.annotations)
    print("\n" + "=" * 20 + "\n")
    agent_resp = await agent.async_run("请帮我将这张中的汽车分割出来", files=[seg_file])
    print("=" * 10 + "分割返回结果" + "=" * 10 + "\n")
    print(agent_resp.text)
    print(agent_resp.annotations)
    print("\n" + "=" * 20 + "\n")
