...
class ResearchTeam:
    ...
    async def _async_run(self, query, iterations=3):
        ...
        for researcher in self.research_actor_instance:
            report = await researcher._async_run(query)
            list_reports.append(report)
        if self.user_agent is not None:
            prompt = (
                f"请你从{self.research_actor_instance}个待选的多个报告草稿中，选择一个合适的报告,"
                + "直接输入序号即可，输入的序号在1和"
                + self.research_actor_instance
                + "之间。"
            )
            index = await self.user_agent.async_run(prompt)
