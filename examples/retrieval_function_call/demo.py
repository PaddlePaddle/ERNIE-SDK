...
from pipelines.nodes import (SpacyTextSplitter, EmbeddingRetriever, ErnieBot,
                             PDFToTextConverter)
...
parser.add_argument(
    "--index_name",
    default='construct_demo_index',
    type=str,
    help="The ann index name of ANN.")
parser.add_argument(
    "--file_paths",
    default='./construction_regulations',
    type=str,
    help="The PDF file path.")
parser.add_argument(
    "--retriever_top_k",
    default=5,
    type=int,
    help="Number of recall items for search")
parser.add_argument(
    "--chunk_size",
    default=384,
    type=int,
    help="The length of data for indexing by retriever")
parser.add_argument(
    '--host',
    type=str,
    default="localhost",
...
parser.add_argument(
    '--port', type=int, default=8081, help='host ip of ANN search engine')
parser.add_argument("--api_key", default=None, type=str, help="The API Key.")
parser.add_argument(
    "--secret_key", default=None, type=str, help="The secret key.")
...
# 如本地语义检索索引已经存在，则直接读取已经构建好的索引
    ...
    indexing_pipeline.add_node(
        component=pdf_converter, name="pdf_converter", inputs=["File"])
    indexing_pipeline.add_node(
        component=text_splitter, name="Splitter", inputs=["pdf_converter"])
    indexing_pipeline.add_node(
        component=retriever, name="Retriever", inputs=["Splitter"])
    indexing_pipeline.add_node(
    ...
    "examples": [
        ...
        },
        {
            ...
            "function_call": {
                "name": "search_knowledge_base",
                "thoughts":
                "这是和城市建设法规标准相关的问题，我需要查询住房和城乡建设部规章，并且设置query为'企业申请建筑业企业资质需要的材料'",
                "arguments": '{ "query": "企业申请建筑业企业资质需要哪些材料？"}',
            },
        },
        {
            "role": "user",
            "content": "历史文化街区的城市设计有什么要求？"
        },
        {
            "role": "assistant",
            "content": None,
            "function_call": {
                "name": "search_knowledge_base",
                "thoughts":
                "这是和城市建设法规标准相关的问题，我需要查询住房和城乡建设部规章，并且设置query为'历史文化街区的设计要求'",
...
def history_transform(history=[]):
    ...
    for turn_idx in range(1, len(history)):
        messages.extend([{
            "role": "user",
            "content": history[turn_idx][0]
        }, {
            "role": "assistant",
            "content": history[turn_idx][1]
        }])
    return messages
...
def prediction(history):
    ...
    # 如果function call触发
        ...
        response = erniebot.ChatCompletion.create(
            model="ernie-bot-3.5", messages=messages)
        result = response["result"]
...
def launch_ui():
    with gr.Blocks(
            title="ERNIE Bot 城市建设法规标准小助手", theme=gr.themes.Base()) as demo:
        gr.HTML("""<h1 align="center">ERNIE Bot 城市建设法规标准小助手</h1>""")
        with gr.Column():
            chatbot = gr.Chatbot(
                value=[[
                    None,
                    "您好, 我是 ERNIE Bot 城市建设法规标准小助手。除了普通的大模型能力以外，还特别了解住房和城乡建设部规章哦"
                ]],
                scale=35,
                height=500)
            message = gr.Textbox(
                placeholder="哪些建筑企业资质需要国务院住房城乡建设主管部门许可？", lines=1, max_lines=20)
            with gr.Row():
