...
from pipelines.nodes import (EmbeddingRetriever, ErnieBot, PDFToTextConverter,
                             SpacyTextSplitter)
...
parser.add_argument(
    "--retriever_top_k", default=5, type=int, help="Number of recall items for search"
)
parser.add_argument(
    "--chunk_size",
    default=384,
...
parser.add_argument(
    "--host", type=str, default="localhost", help="host ip of ANN search engine"
)
parser.add_argument(
...
def launch_ui():
    with gr.Blocks(title="ERNIE Bot 城市建设法规标准小助手", theme=gr.themes.Base()) as demo:
        ...
        with gr.Column():
            chatbot = gr.Chatbot(
                value=[
                    [None, "您好, 我是 ERNIE Bot 城市建设法规标准小助手。除了普通的大模型能力以外，还特别了解住房和城乡建设部规章哦"]
                ],
                scale=35,
                height=500,
            )
            ...
            log = gr.JSON()
        message.submit(
            add_message_chatbot, inputs=[message, chatbot], outputs=[message, chatbot]
        ).then(prediction, inputs=[chatbot], outputs=[chatbot, log])
        submit.click(
            add_message_chatbot, inputs=[message, chatbot], outputs=[message, chatbot]
        ).then(prediction, inputs=[chatbot], outputs=[chatbot, log])
        clear.click(
