...
def create_ui_and_launch(args):
    with gr.Blocks(
        ...
        theme=gr.themes.Soft(spacing_size="sm", text_size="md"),
    ) as block:
            ...
            with gr.Row():
                ...
                with gr.Column(scale=3):
                    ...
                    input_text = gr.Textbox(
                        label="消息内容", value="请问12和16的“魔法运算”结果是多少？", placeholder="请输入..."
                    )
                    with gr.Row():
                        ...
                        with gr.Tab(label="自定义函数"):
                            ...
                            custom_func_desc = JSONCode(
                                ...
                                value=to_pretty_json(
                                    get_custom_func_desc_template(), from_json=False
                                ),
                                interactive=True,
                            )
...
def create_function_tab(function):
    with gr.Tab(label=function.name):
        with gr.Column():
            ...
            JSONCode(
                label="描述",
                value=to_pretty_json(function.desc, from_json=False),
                interactive=False,
            )
...
def generate_response_for_function(
    ...
    message = {
        "role": "function",
        "name": func_name,
        "content": to_compact_json(func_res, from_json=True),
    }
...
def try_update_candidates(state, candidates, custom_func_code, custom_func_desc_str):
    if CUSTOM_FUNC_NAME in candidates:
        try:
            custom_function = make_custom_function(
                custom_func_code, custom_func_desc_str
            )
        except Exception as e:
            handle_exception(
                e, f"自定义函数的定义或描述中存在错误，无法将其添加为候选函数。错误信息如下：{str(e)}", raise_=False
            )
